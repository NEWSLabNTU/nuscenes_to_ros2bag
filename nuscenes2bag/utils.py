...
def create_topics(nusc, scene, writer):
    ...
    for (sensor_id, sample_token) in sample["data"].items():
        ...
        topic = "/" + sensor_id
        if sample_data["sensor_modality"] == "radar" or sample_data["sensor_modality"] == "lidar":
            writer.create_topic(rosbag2_py.TopicMetadata(
                name=topic, type="sensor_msgs/msg/PointCloud2", serialization_format="cdr"
            ))  
        elif sample_data["sensor_modality"] == "camera":
            writer.create_topic(rosbag2_py.TopicMetadata(
                name=topic + "/image_rect_compressed", type="sensor_msgs/msg/CompressedImage", serialization_format="cdr"
            ))
            writer.create_topic(rosbag2_py.TopicMetadata(
                name=topic + "/camera_info", type="sensor_msgs/msg/CameraInfo", serialization_format="cdr"
            ))
            writer.create_topic(rosbag2_py.TopicMetadata(
